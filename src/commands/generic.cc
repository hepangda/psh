...
int GenericCommand::execute(Context &ctx, Expression &cmd) {
    ...
    arglist[cmd.arguments.size()] = (char *)nullptr;  // Ensure that the parameter list is terminated with NULL
    ...
    // Parent process
    if (child > 0) {
        // If it is a background process, let it run independently. Output its ID and continue reading the next command.
...
SearchResult GenericCommand::findExecfile(Context &ctx, std::string_view execfile) {
    // If the command is not a built-in command, then sequentially search for the executable file of the command in the
